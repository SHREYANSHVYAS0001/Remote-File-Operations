...
# def load_balancer():
#     response = requests.get(f"{SERVER_URL}/load_balancer")
#     if response.status_code == 200:
#         content = response.text
#         print(f"Content of {filename}:\n{content}")
#     else:
...
#         response = requests.post(f"{SERVER_URL}/load_balancer")
        
#         # Check if the request was successful
#         if response.status_code == 200:
#             data = response.json()
#             return data.get('current_server_url', 'No server URL found')
#         else:
...
def load_balancer():
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            new_server_url = data.get('current_server_url', None)
            if new_server_url:
                SERVER_URL = new_server_url
                print(f"Load balancer updated server URL to: {SERVER_URL}")
            else:
                print("No server URL found in response.")
        else:
            print(f"Failed to get server URL from load balancer. Status code: {response.status_code}")
    except requests.RequestException as e:
